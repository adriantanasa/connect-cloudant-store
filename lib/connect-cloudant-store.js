...
module.exports = function(session) {
    ...
    CloudantStore.prototype.get = function(sid, fn) {
        debug('GET "%s" pid "%s"', sid, process.pid);
        fn = fn || noop;

        self.db.get(self.prefix + sid)
        .then(function(data) {
            if (data.session_modified + data.session_ttl * 1000 < Date.now()) {
                debug('GET "%s" pid "%s" expired session', sid, process.pid);
                self.destroy(sid, fn);
                return fn();
            } else {
                debug('GET "%s" pid "%s" found rev "%s"', sid, process.pid, data._rev);
                return fn(null, data);
            }
        })
        .catch(function(err) {
            if (err.statusCode == 404) {
                debug('GET - SESSION NOT FOUND "%s" pid "%s"', sid, process.pid);
                return fn();
            } else {
                // TODO 429 errors from a custom promise-retry cloudant plugin
                debug('GET ERROR  "%s" pid "%s" err "%s"', sid, process.pid,
                    JSON.stringify(err));
                self.emit('error', err);
                return fn(err);
            }
    ...
    CloudantStore.prototype.set = function(sid, sess, fn) {
        ...
        self.db.head(self.prefix + sid, function(err, _, headers) {
            ...
            if (!err && headers && headers.etag) {
                ...
                debug('SET session error "%s" rev "%s" pid "%s" err "%s"', sid, sess._rev, process.pid,
                    JSON.stringify(err));
                self.emit('error', err);
                return fn(err);
            }

            debug('SET session "%s" rev "%s" pid "%s"', sid, sess._rev, process.pid);
    ...
    CloudantStore.prototype.destroy = function(sid, fn) {
        debug('DESTROY session "%s" pid "%s"', sid, process.pid);
        fn = fn || noop;
        ...
        self.db.get(self.prefix + sid).then(function(data) {
            ...
            .catch(function(err) {
                debug('DESTROY - DB error "%s" rev "%s" pid "%s" err "%s"', sid, data._rev, process.pid,
                JSON.stringify(err));
                self.emit('error', err);
                return fn(err);
            });
        }).catch(function(err) {
            debug('DESTROY - DB GET failure "%s" pid "%s" err "%s"', sid, process.pid, JSON.stringify(err));
            self.emit('error', err);
            return fn(err);
        });
    };
    ...
    CloudantStore.prototype.touch = function(sid, sess, fn) {
        ...
        debug('TOUCH session "%s" rev "%s" pid "%s"', sid, sess._rev, process.pid);
        ...
        .then(function(data) {
            ...
            .catch(function(err) {
                debug('TOUCH session error "%s" rev "%s" pid "%s" err "%s"', sid, sess._rev, process.pid,
                    JSON.stringify(err));
                self.emit('error', err);
                return fn(err);
            });
        ...
        .catch(function(err) {
            debug('TOUCH - error on returning the session "%s" rev "%s" pid "%s" err "%s"',
                sid, sess._rev, process.pid, JSON.stringify(err));
            self.emit('error', err);
            return fn(err);
        });
    };
